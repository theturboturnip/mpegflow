...
mpegflow: mpegflow.cpp
	@mkdir -p $(BUILD_DIR)
	g++ $< -o $(BUILD_DIR)/$@ $(CFLAGS) $(LDFLAGS) $(INSTALLED_DEPS)

vis: vis.cpp
	@mkdir -p $(BUILD_DIR)
	g++ $< -o $(BUILD_DIR)/$@ $(CFLAGS) -lopencv_highgui -lopencv_videoio -lopencv_imgproc -lopencv_imgcodecs -lopencv_core -lpng $(LDFLAGS) $(INSTALLED_DEPS) 
